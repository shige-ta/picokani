...
func (c *DiscordChannel) handleMessage(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if !c.isChannelAllowed(m.ChannelID) {
		logger.DebugCF("discord", "Message rejected by channel allowlist", map[string]any{
			"channel_id": m.ChannelID,
		})
		return
	}

	if c.config.MentionOnly && m.GuildID != "" && !c.isBotMentioned(s, m) {
		return
	}
...
func (c *DiscordChannel) isChannelAllowed(channelID string) bool {
	if len(c.config.AllowChannels) == 0 {
		return true
	}
	for _, allowed := range c.config.AllowChannels {
...
func (c *DiscordChannel) isBotMentioned(s *discordgo.Session, m *discordgo.MessageCreate) bool {
	if m == nil || s == nil || s.State == nil || s.State.User == nil {
		return false
	}
